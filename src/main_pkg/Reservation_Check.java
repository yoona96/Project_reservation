...
public class Reservation_Check {
	...
	public void input_inform(String section) {
			...
			for (int j = 0; j < 20; j++) {
				for (int k = 0; k < 11; k++) {
					if (user_name.equals(db[4][k][j]) && phone_num.equals(db[5][k][j])) {
						int[] pos = new int[2];
						pos[0] = k; // pos[0]: 예약 시간에 해당합니다. 
						pos[1] = j; // pos[1]: 테이블 번호에 해당합니다.
	...
	public ArrayList[] show_reservation_inform(String section) { // Check, Cancle, Delete 중 하나
		...
		for (int i = 0; i < 3; i++) {
			...
			if (user_inform[i].size() == 0)
				continue;
			for (int j = 0; j < user_inform[i].size() / 2; j += 2) {
